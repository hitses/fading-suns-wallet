...
export default class Money implements OnInit, OnDestroy {
  ...
  updateFenix(value: number): void {
    if (value > 0) this.incrementAttribute('fenix', value);
    else this.decrementAttribute('fenix', -value);

    this.saveCharacter();
  }

  updateBlason(value: number): void {
    if (value > 0) {
  ...
  updateCresta(value: number): void {
    if (value > 0) {
